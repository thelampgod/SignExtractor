...
public class Main {
    ...
    public static void main(String... args) throws IOException {
        ...
        try (FileWriter writer = new FileWriter(output + "/signs.txt")) {
            ...
            Arrays.asList(new File(regionFolder).listFiles()).stream()
                    ...
                    .forEach(file -> {
                        ...
                        mca.forEach(chunk -> {
                            if (chunk == null || chunk.getData() == null) return;

                            chunk.getData().getList("block_entities").stream()
                                    .map(tag -> (CompoundTag) tag)
                                    .forEach(tile -> {
                                        if (!tile.getString("id").equals("minecraft:sign")) {
                                            return;
                                        }
